...
std::shared_ptr<endpoint> routing_manager_impl::create_client_endpoint(
        ...
        bool _reliable, client_t _client, secure_channel_t _secure_channel) {
    (void)_client;
        ...
        if (_start) {
            if (_reliable) {
                ...
                if (_secure_channel != ILLEGAL_CHANNEL && configuration_->is_authentic(_secure_channel) &&
                    ...
                    its_endpoint = std::make_shared<udp_server_endpoint_impl>(shared_from_this(), ep, io_, its_limit);
                }
            }
